...
INTF=$(ip link | grep 'lxc-br0' | grep 'state UP')
while [ -z "$INTF" ]; do
    echo "Waiting for lxc-br0 interface to become available..."
    sleep 1
    INTF=$(ip link | grep 'lxc-br0' | grep 'state UP')
done
...
ip addr add "${OPENWRT_LOCAL_IP}/${LXCNETWORKMASKLEN}" dev lxc-br0
ip route add default via "${LXCGATEWAY}"
